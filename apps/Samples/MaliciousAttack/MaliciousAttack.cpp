...
// Kernel for matrix transposition
// One sub-square at a time
template <int SquareSize> struct MaliciousTranspose : Kernel {
  Array2D<int> outshit;
  ...
  void kernel() {
    ...
    // square += BANKED_SRAMS_SIZE_HALF;
    __syncthreads();
    
    // Store square
    for (int y = threadIdx.y; y < blockDim.x; y += blockDim.y)
      outshit[originX + y][originY + threadIdx.x] = square[threadIdx.x][y];
      // outshit[originX + y][originY + threadIdx.x] = 1;
  }
};
...
int main()
{
  ...
  // Instantiate kernel
  MaliciousTranspose<SIMTLanes> k1;
  ...
  // Invoke kernel
  noclRunOverlappingKernelAndDumpStats(&k2, &k1);
  ...
  for (int i = 0; i < width; i++)
    for (int j = 0; j < height; j++)
    {
      ...
      printf("first: %x, second: %x\n", matOutFirst[i][j], matOutSecond[i][j]);
      okFirst = okFirst && matOutFirst[i][j] == matInSecond[j][i];
